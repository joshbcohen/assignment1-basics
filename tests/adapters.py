...
from cs336_basics.optimizer import AdamW, get_learning_rate_schedule
...
def run_linear(
    ...
    l = Linear(d_in, d_out)
    test_dict = {"W": weights}
    l.load_state_dict(test_dict)
    out = l.forward(in_features)
...
def run_embedding(
    ...
    test_dict = {"embedding_matrix": weights}
    e.load_state_dict(test_dict)
...
def run_multihead_self_attention_with_rope(
    ...
    multihead_self_attention_with_rope.load_state_dict(
        {"W_Q": q_proj_weight, "W_K": k_proj_weight, "W_V": v_proj_weight, "W_O": o_proj_weight}
    )
    return multihead_self_attention_with_rope.forward(in_features, token_positions)
...
def run_get_lr_cosine_schedule(
    ...
    return get_learning_rate_schedule(
        t=it, a_max=max_learning_rate, a_min=min_learning_rate, T_w=warmup_iters, T_c=cosine_cycle_iters
    )
